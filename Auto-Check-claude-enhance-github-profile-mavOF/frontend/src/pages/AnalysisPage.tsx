      ...
      <div className="bg-white shadow-sm border-b">
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{deck.filename}</h1>
              <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
                <span>{deck.metadata.slide_count} slides</span>
                <span>•</span>
      ...
      <div className="flex h-[calc(100vh-120px)]">
        ...
        <div className="w-64 bg-white border-r overflow-y-auto">
          <div className="p-4">
            ...
            <div className="space-y-2">
              {Array.from({ length: deck.metadata.slide_count }).map((_, idx) => (
                <div
                  key={idx}
                ...
                >
                  ...
                  {issuesData && (
                    <div className="text-xs text-gray-500 mt-1">
                      {issuesData.issues.filter((i: any) => i.location.slide_number === idx + 1).length} issues
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="w-96 bg-white border-l flex flex-col">
          ...
          <div className="flex border-b">
            ...
            >
              Changes ({operationsData?.total || 0})
            </button>
          </div>
            ...
            {isAnalyzing ? (
              ...
              <>
                {activeTab === 'issues' && issuesData && (
                  <IssuesPanel issues={issuesData.issues} onRefresh={refetchDeck} />
                )}
                {activeTab === 'changes' && operationsData && (
                  <ChangesPanel operations={operationsData.operations} onRefresh={refetchDeck} />
                )}
              </>
            )}
          </div>
